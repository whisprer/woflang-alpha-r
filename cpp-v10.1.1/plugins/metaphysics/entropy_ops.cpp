// ===============================================================
// entropy_ops.cpp - All operations currently just log that they are unimplemented.
...
extern "C" WOFLANG_PLUGIN_EXPORT void register_plugin(WoflangInterpreter& interp) {
    ...
    interp.register_op("chaos", [](WoflangInterpreter& ip) {
        std::cout << "[entropy_ops] op \"chaos\" is not yet implemented."
                  << " Stack size: " << ip.stack.size() << "\n";
    });

    interp.register_op("entropy", [](WoflangInterpreter& ip) {
        std::cout << "[entropy_ops] op \"entropy\" is not yet implemented."
                  << " Stack size: " << ip.stack.size() << "\n";
    });

    interp.register_op("order", [](WoflangInterpreter& ip) {
        std::cout << "[entropy_ops] op \"order\" is not yet implemented."
                  << " Stack size: " << ip.stack.size() << "\n";
    });
