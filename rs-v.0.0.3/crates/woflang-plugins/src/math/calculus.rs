...
pub fn register(interp: &mut Interpreter) {
    ...
    interp.register("derivative_central", |interp| {
        ...
        let f_minus = interp.stack_mut().pop()?.as_float()?;
        let f_plus = interp.stack_mut().pop()?.as_float()?;
    ...
    interp.register("trapezoid", |interp| {
        let h = interp.stack_mut().pop()?.as_float()?;
        let f_b = interp.stack_mut().pop()?.as_float()?;
        let f_a = interp.stack_mut().pop()?.as_float()?;
    ...
    interp.register("simpson", |interp| {
        let h = interp.stack_mut().pop()?.as_float()?;
        let f_b = interp.stack_mut().pop()?.as_float()?;
        let f_m = interp.stack_mut().pop()?.as_float()?;
        let f_a = interp.stack_mut().pop()?.as_float()?;
    ...
    interp.register("∏", |interp| {
        ...
        for _ in 0..n {
            product *= interp.stack_mut().pop()?.as_float()?;
        }
