...
pub fn register(interp: &mut Interpreter) {
    ...
    interp.register("solve_quadratic", |interp| {
        ...
        match solve_quadratic(a, b, c) {
            QuadraticResult::TwoReal(x1, x2) => {
                ...
                println!("  x₁ = {}", x1);
                println!("  x₂ = {}", x2);
                interp.stack_mut().push(WofValue::Float(x1));
                interp.stack_mut().push(WofValue::Float(x2));
            }
    ...
    interp.register("solve_linear_2x2", |interp| {
        let c2 = interp.stack_mut().pop()?.as_float()?;
        let b2 = interp.stack_mut().pop()?.as_float()?;
        let a2 = interp.stack_mut().pop()?.as_float()?;
        let c1 = interp.stack_mut().pop()?.as_float()?;
        let b1 = interp.stack_mut().pop()?.as_float()?;
        let a1 = interp.stack_mut().pop()?.as_float()?;
    ...
    interp.register("newton_sqrt", |interp| {
        ...
        match newton_raphson(f, f_prime, n / 2.0, 100, 1e-15) {
            Some(root) => {
                ...
                interp.stack_mut().push(WofValue::Float(root));
            }
            None => {
                println!("[solver] Newton-Raphson failed to converge");
                interp.stack_mut().push(WofValue::Float(n.sqrt()));
            }
        }
        Ok(())
    });
    ...
    interp.register("newton_cbrt", |interp| {
        ...
        match newton_raphson(f, f_prime, x0, 100, 1e-15) {
            ...
            None => {
                println!("[solver] Newton-Raphson failed to converge");
                interp.stack_mut().push(WofValue::Float(n.cbrt()));
            }
        }
        Ok(())
    });
