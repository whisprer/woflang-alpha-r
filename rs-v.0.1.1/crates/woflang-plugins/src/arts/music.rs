...
pub fn register(interp: &mut Interpreter) {
    ...
    interp.register("interval_info", |interp| {
        let upper = interp.stack_mut().pop()?.as_integer()? as i32;
        let lower = interp.stack_mut().pop()?.as_integer()? as i32;
    ...
    interp.register("midi_name", |interp| {
        let midi = interp.stack_mut().pop()?.as_integer()? as i32;
    ...
    interp.register("note_freq", |interp| {
        ...
        let (midi, a4) = if let Ok(top) = interp.stack().peek() {
            ...
            let a4 = val.as_double().unwrap_or(440.0);
            let midi = interp.stack_mut().pop()?.as_integer()? as i32;
            (midi, a4)
        } else {
            // One value: just midi, use default A4=440
            let midi = val.as_integer()? as i32;
            (midi, 440.0)
        };
        
        let freq = midi_to_freq(midi, a4);
        interp.stack_mut().push(WofValue::double(freq));
        Ok(())
    });
    ...
    interp.register("edo_freq", |interp| {
        let base = interp.stack_mut().pop()?.as_double()?;
        let edo = interp.stack_mut().pop()?.as_integer()? as i32;
        let degree = interp.stack_mut().pop()?.as_integer()? as i32;
    ...
    interp.register("bpm_ms", |interp| {
        ...
        let (bpm, div) = if let Ok(s) = val.as_string() {
            ...
            let bpm = interp.stack_mut().pop()?.as_double()?;
            (bpm, s)
        } else {
            // Just BPM
            (val.as_double()?, "1/4".to_string())
        };
        ...
        interp.stack_mut().push(WofValue::double(quarter_ms * factor));
        Ok(())
    });
    ...
    interp.register("euclid_pattern", |interp| {
        let steps = interp.stack_mut().pop()?.as_integer()? as i32;
        let pulses = interp.stack_mut().pop()?.as_integer()? as i32;
    ...
    interp.register("polyrhythm", |interp| {
        let b = interp.stack_mut().pop()?.as_integer()? as i32;
        let a = interp.stack_mut().pop()?.as_integer()? as i32;
    ...
    interp.register("swing_ms", |interp| {
        let swing = interp.stack_mut().pop()?.as_double()?;
        let bpm = interp.stack_mut().pop()?.as_double()?;
        ...
        interp.stack_mut().push(WofValue::double(delta));
        Ok(())
    });
