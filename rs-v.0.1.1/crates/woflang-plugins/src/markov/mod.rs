        ...
        let suggestion = match stack_len {
            ...
            1 => {
                ...
                if let Ok(top) = interp.stack().peek() {
                    if top.is_integer() && top.try_integer().unwrap_or(0) > 0 {
                        "Try: fact (factorial), sqrt, dup *"
                    } else if top.is_double() && top.try_double().unwrap_or(0.0) >= 0.0 {
                        "Try: sqrt, sin, cos, round"
                    } else if top.try_str().is_some() {
                        "Try: print, strlen, upper"
                    } else {
                        "Try: dup, print"
                    }
                } else {
