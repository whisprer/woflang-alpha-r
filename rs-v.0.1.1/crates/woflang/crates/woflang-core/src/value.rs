...
impl WofValue {
    ...
    /// Extract as owned String. Convenience alias for plugins.
    #[inline]
    pub fn as_string(&self) -> Result<String> {
        self.as_str().map(|s| s.to_owned())
    }

    /// Extract as f64. Alias for [`as_double`](Self::as_double).
    #[inline]
    pub fn as_float(&self) -> Result<f64> {
        self.as_double()
    }

    /// Returns `true` if this value is nil/unknown.
    #[inline]
    #[must_use]
    pub const fn is_nil(&self) -> bool {
        matches!(self.typ, WofType::Unknown)
    }

    /// Returns `true` if this value is a string (not symbol).
    #[inline]
    #[must_use]
    pub const fn is_string(&self) -> bool {
        matches!(self.typ, WofType::String)
    }
