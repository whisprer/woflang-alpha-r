...
pub fn register(interp: &mut Interpreter) {
    ...
    interp.register("copysign", |interp| {
        let sign = interp.stack_mut().pop()?.as_double()?;
        let mag = interp.stack_mut().pop()?.as_double()?;
    ...
    interp.register("clamp", |interp| {
        let hi = interp.stack_mut().pop()?.as_double()?;
        let lo = interp.stack_mut().pop()?.as_double()?;
        let x = interp.stack_mut().pop()?.as_double()?;
