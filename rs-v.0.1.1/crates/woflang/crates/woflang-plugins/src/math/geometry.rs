...
pub fn register(interp: &mut Interpreter) {
    ...
    interp.register("translate2d", |interp| {
        let dy = interp.stack_mut().pop()?.as_double()?;
        let dx = interp.stack_mut().pop()?.as_double()?;
        let y = interp.stack_mut().pop()?.as_double()?;
        let x = interp.stack_mut().pop()?.as_double()?;

        interp.stack_mut().push(WofValue::double(x + dx));
        interp.stack_mut().push(WofValue::double(y + dy));
        Ok(())
    });

    // Scale: x y sx sy → x*sx y*sy
    interp.register("scale2d", |interp| {
        let sy = interp.stack_mut().pop()?.as_double()?;
        let sx = interp.stack_mut().pop()?.as_double()?;
        let y = interp.stack_mut().pop()?.as_double()?;
        let x = interp.stack_mut().pop()?.as_double()?;

        interp.stack_mut().push(WofValue::double(x * sx));
        interp.stack_mut().push(WofValue::double(y * sy));
        Ok(())
    });

    // Rotate by radians: x y theta → x' y'
    interp.register("rotate2d_rad", |interp| {
        let theta = interp.stack_mut().pop()?.as_double()?;
        let y = interp.stack_mut().pop()?.as_double()?;
        let x = interp.stack_mut().pop()?.as_double()?;
        ...
        interp.stack_mut().push(WofValue::double(x_new));
        interp.stack_mut().push(WofValue::double(y_new));
        Ok(())
    });

    // Rotate by degrees: x y theta_deg → x' y'
    interp.register("rotate2d_deg", |interp| {
        let theta_deg = interp.stack_mut().pop()?.as_double()?;
        let y = interp.stack_mut().pop()?.as_double()?;
        let x = interp.stack_mut().pop()?.as_double()?;
    ...
    interp.register("cart_to_polar", |interp| {
        ...
        interp.stack_mut().push(WofValue::double(r));
        interp.stack_mut().push(WofValue::double(theta));
        Ok(())
    });
    ...
    interp.register("polar_to_cart", |interp| {
        let theta = interp.stack_mut().pop()?.as_double()?;
        let r = interp.stack_mut().pop()?.as_double()?;
    ...
    interp.register("vec2_mag", |interp| {
        let y = interp.stack_mut().pop()?.as_double()?;
        let x = interp.stack_mut().pop()?.as_double()?;
        interp.stack_mut().push(WofValue::double((x * x + y * y).sqrt()));
        Ok(())
    });
    ...
    interp.register("vec2_dot", |interp| {
        ...
        interp.stack_mut().push(WofValue::double(x1 * x2 + y1 * y2));
        Ok(())
    });
    ...
    interp.register("vec2_normalize", |interp| {
        ...
        if mag > f64::EPSILON {
            interp.stack_mut().push(WofValue::double(x / mag));
            interp.stack_mut().push(WofValue::double(y / mag));
        } else {
    ...
    interp.register("vec2_dist", |interp| {
        ...
        let dx = x2 - x1;
        let dy = y2 - y1;
        interp.stack_mut().push(WofValue::double((dx * dx + dy * dy).sqrt()));
        Ok(())
    });

    // Lerp (linear interpolation): x1 y1 x2 y2 t → x y
    interp.register("vec2_lerp", |interp| {
        let t = interp.stack_mut().pop()?.as_double()?;
        let y2 = interp.stack_mut().pop()?.as_double()?;
        let x2 = interp.stack_mut().pop()?.as_double()?;
        let y1 = interp.stack_mut().pop()?.as_double()?;
        let x1 = interp.stack_mut().pop()?.as_double()?;
