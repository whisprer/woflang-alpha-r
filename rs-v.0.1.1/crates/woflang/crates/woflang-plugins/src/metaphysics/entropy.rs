...
fn make_entropy_key(v: &WofValue) -> String {
    if let Some(n) = v.try_integer() { format!("i:{}", n) }
    else if let Some(f) = v.try_double() { format!("f:{:.10}", f) }
    else if let Some(s) = v.try_str() { format!("s:{}", s) }
...
pub fn register(interp: &mut Interpreter) {
    ...
    interp.register("entropy_max", |interp| {
        ...
        if n == 0 {
            ...
            interp.stack_mut().push(WofValue::double(h_max));
        }
        Ok(())
    });
