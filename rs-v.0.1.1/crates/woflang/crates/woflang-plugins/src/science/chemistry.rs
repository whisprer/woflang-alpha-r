...
pub fn register(interp: &mut Interpreter) {
    ...
    interp.register("element_info", |interp| {
        ...
        let elem = if let Some(s) = key.try_str() { find_element(s) } else if let Ok(n) = key.as_double() { find_by_z(n as i32) } else { None };
    ...
    interp.register("atomic_weight", |interp| {
        ...
        interp.stack_mut().push(WofValue::double(weight));
        Ok(())
    });
    ...
    interp.register("temp_convert", |interp| {
        ...
        let (value, mode) = if let Ok(s) = top.as_string() {
            (next.as_double().unwrap_or(0.0), s)
        } else if let Ok(s) = next.as_string() {
            (top.as_double().unwrap_or(0.0), s)
        } else {
    ...
    interp.register("c_to_f", |interp| {
        ...
        interp.stack_mut().push(WofValue::double(c * 9.0 / 5.0 + 32.0));
        Ok(())
    });

    interp.register("f_to_c", |interp| {
        let f = interp.stack_mut().pop()?.as_double()?;
        interp.stack_mut().push(WofValue::double((f - 32.0) * 5.0 / 9.0));
        Ok(())
    });

    interp.register("c_to_k", |interp| {
        let c = interp.stack_mut().pop()?.as_double()?;
        interp.stack_mut().push(WofValue::double(c + 273.15));
        Ok(())
    });

    interp.register("k_to_c", |interp| {
        let k = interp.stack_mut().pop()?.as_double()?;
        interp.stack_mut().push(WofValue::double(k - 273.15));
        Ok(())
    });
    ...
    interp.register("avogadro", |interp| {
        interp.stack_mut().push(WofValue::double(AVOGADRO));
        Ok(())
    });

    // Gas constant
    interp.register("gas_constant", |interp| {
        interp.stack_mut().push(WofValue::double(GAS_CONSTANT));
        Ok(())
    });

    // Faraday constant
    interp.register("faraday", |interp| {
        interp.stack_mut().push(WofValue::double(FARADAY));
        Ok(())
    });
