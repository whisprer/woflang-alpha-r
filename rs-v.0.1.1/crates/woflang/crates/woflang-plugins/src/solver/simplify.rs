...
fn is_zero(v: &WofValue) -> bool {
    if let Some(n) = v.try_integer() { return n == 0; }
    if let Some(f) = v.try_double() { return f == 0.0; }
    false
}

/// Check if a value is one.
fn is_one(v: &WofValue) -> bool {
    if let Some(n) = v.try_integer() { return n == 1; }
    if let Some(f) = v.try_double() { return (f - 1.0).abs() < f64::EPSILON; }
    false
}

/// Get string content from a value.
fn as_string(v: &WofValue) -> Option<String> {
    v.try_str().map(|s| s.to_owned())
}

/// Check if two values represent the same symbol.
fn same_symbol(a: &WofValue, b: &WofValue) -> bool {
    match (a.try_str(), b.try_str()) {
