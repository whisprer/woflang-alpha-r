...
fn default_state_dir() -> Result<PathBuf> {
    if let Ok(p) = std::env::var("TORVPN_STATE_DIR") {
        if !p.trim().is_empty() {
...
async fn start(cfg: config::Config, state_dir: &PathBuf) -> Result<()> {
    ...
    // 4) Start tun2socks (optional)
    let t2s = if cfg.tun2socks.enabled {
        Some(tun2socks_manager::Tun2Socks::start(&cfg).await?)
    } else {
        None
    };
    ...
    let pids = serde_json::json!({
        "tor": tor.pid(),
        "tun2socks": t2s.as_ref().map(|x| x.pid()).unwrap_or(0),
    });
    ...
    if let Some(t2s) = &t2s {
        ...
        println!("torvpn-win: started (Tor PID {}, tun2socks disabled)", tor.pid());
    }
    Ok(())
}
...
async fn control_connect(state_dir: &std::path::Path, cfg: &config::Config) -> anyhow::Result<tor_control::TorControl> {
    let cookie = if let Ok(p) = std::env::var("TORVPN_COOKIE_PATH") {
        if !p.trim().is_empty() { std::path::PathBuf::from(p) } else { state_dir.join("tor-data").join("control_auth_cookie") }
    } else {
