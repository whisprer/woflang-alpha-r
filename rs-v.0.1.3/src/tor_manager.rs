...
fn generate_torrc(cfg: &Config, data_dir: &Path) -> Result<String> {
    ...
    // DNSPort is optional on Windows (5353 is often in use). If disabled (0),
    // clients can still use SOCKS and do their own DNS.
    if dns != 0 {
...
pub async fn apply_proxy_and_exit(cfg: &crate::config::Config, state_dir: &std::path::Path) -> anyhow::Result<()> {
    ...
    } else { state_dir.join("tor-data").join("control_auth_cookie") };
