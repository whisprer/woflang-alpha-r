...
pub fn register(interp: &mut Interpreter) {
    ...
    interp.register("mandelbrot", |interp| {
        let max_iter = interp.stack_mut().pop()?.as_integer()?.clamp(1, 10000) as i32;
        let imag = interp.stack_mut().pop()?.as_double()?;
        let real = interp.stack_mut().pop()?.as_double()?;
    ...
    interp.register("julia", |interp| {
        let max_iter = interp.stack_mut().pop()?.as_integer()?.clamp(1, 10000) as i32;
        let ci = interp.stack_mut().pop()?.as_double()?;
        let cr = interp.stack_mut().pop()?.as_double()?;
        let zi = interp.stack_mut().pop()?.as_double()?;
        let zr = interp.stack_mut().pop()?.as_double()?;
    ...
    interp.register("sierpinski", |interp| {
        let depth = interp.stack_mut().pop()?.as_integer()? as i32;
    ...
    interp.register("hausdorff_dim", |interp| {
        let scale = interp.stack_mut().pop()?.as_double()?;
    ...
    interp.register("mandelbrot_ascii", |interp| {
        let max_iter = interp.stack_mut().pop()?.as_integer()?.clamp(1, 100) as i32;
        let height = interp.stack_mut().pop()?.as_integer()?.clamp(1, 50) as i32;
        let width = interp.stack_mut().pop()?.as_integer()?.clamp(1, 100) as i32;
        let y_max = interp.stack_mut().pop()?.as_double()?;
        let y_min = interp.stack_mut().pop()?.as_double()?;
        let x_max = interp.stack_mut().pop()?.as_double()?;
        let x_min = interp.stack_mut().pop()?.as_double()?;
